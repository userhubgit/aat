...
public class LuceneIndexRechercheImpl implements LuceneIndexRecherche {
	...
	private void indexDocs(final IndexWriter pWriter, final Motif pMotif) throws Exception {
		if (pMotif != null) {
			...
			doc.add(new Field(CHAMP_CODE, new String(pMotif.getCode().getBytes() , Charset.forName("ANSI_X3.4-1968")), Store.YES, Index.ANALYZED));
			doc.add(new Field(CHAMP_LIBELLE, new String(pMotif.getLibelle().getBytes() , Charset.forName("ANSI_X3.4-1968")), Store.YES, Index.ANALYZED));
...
//					logger.info("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
	...
	private List<Motif> singleTermSearchV0(String pTerm, IndexSearcher pSearcher) throws ParseException, IOException {
		...
		try {
			...
			logger.info("======= { Nombre de motif "+ new String("trouv�".getBytes() , Charset.forName("ANSI_X3.4-1968")) + " := " + hits.length + " } ===========\n");
